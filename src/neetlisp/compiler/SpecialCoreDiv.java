...
public class SpecialCoreDiv implements SpecializedCompile
{
    @Override
    public Name getName()
    {
    ...
    public void compile(Scope scope, CFn cfn, MethodVisitor mv, NFiniteSeq cdr)
    {
        ...
        if(!it.hasNext())
            mv.visitMethodInsn(Opcodes.INVOKEINTERFACE, "neetlisp/Number", "recip", "()Lneetlisp/Number;");
        else
        {
            do
            {
                scope.compiler.compileObject(scope, cfn, mv, it.next());
                mv.visitTypeInsn(Opcodes.CHECKCAST, "neetlisp/Number");
                mv.visitMethodInsn(Opcodes.INVOKEINTERFACE, "neetlisp/Number", "div", "(Lneetlisp/Number;)Lneetlisp/Number;");
            } while(it.hasNext());
        }
    }
}
